...
export default function AdminDashboard() {
  ...
  const recentActivity = useMemo(() => {
    if (audits.length) {
      return audits.slice(0, 5).map(
        (entry) => `${entry.allocationType} mapped to ${entry.assigneeName} on ${entry.unitNo}`
      )
    }
    return users.slice(0, 5).map(
      (entry) => `User onboarded: ${entry.fullName} (${entry.role})`
    )
  }, [audits, users])
    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          ...
          <p className="mt-2 text-3xl font-bold text-teal-700">
            {formatCurrency(
              rentsSeed.reduce((sum, item) => sum + Number(item.amount || 0), 0)
            )}
          </p>
          <p className="mt-1 text-sm text-soft">
            Month-to-date net collections across all properties.
          </p>
        </Card>
        ...
        <Card>
          ...
          <div className="mt-4 space-y-2 text-sm">
            <button
              ...
              className="flex w-full items-center gap-2 rounded-xl border border-base px-3 py-2 hover:bg-slate-50"
            >
            ...
            <button
              type="button"
              disabled
              onClick={() => showToast('Upload ledger module is planned in next phase.', 'info')}
              className="flex w-full cursor-not-allowed items-center gap-2 rounded-xl border border-base px-3 py-2 text-soft opacity-70"
            >
              <Upload size={15} /> Upload Ledger (Coming soon)
            </button>
          </div>
        </Card>
      </section>
      ...
      <Card>
        ...
        <ul className="mt-4 space-y-3 text-sm">
          {recentActivity.map((activity, index) => (
            <li
              key={`${activity}-${index}`}   
              className="rounded-xl border border-base px-3 py-2"
            >
              {activity}
            </li>
          ))}
      ...
      <Card>
        ...
        <p className="mt-4 text-sm text-soft">
          Use the dedicated Users page to add, reset passwords, and remove users.
        </p>
      </Card>
    </div>
  )
